...
leftSpringNode2 = springNodes2(1,1);
leftSNId2 = getId(leftSpringNode2);
rightSpringNode2 = springNodes2(1,2);
rightSNId2 = getId(rightSpringNode2);

spring3ID = length(allElements)+4;
spring4ID = length(allElements)+5;

model.addNewElement('SpringDamperElement3d2n',spring3ID,[leftSNId2 massNodeID]);
model.addNewElement('SpringDamperElement3d2n',spring4ID,[massNodeID rightSNId2]);

model.getNode(leftSNId2).addDof("DISPLACEMENT_Z");
model.getNode(rightSNId2).addDof("DISPLACEMENT_Z");

model.getNode(leftSNId2).fixDof('DISPLACEMENT_Z');
model.getNode(rightSNId2).fixDof('DISPLACEMENT_Z');
...
figure(2)
title('Dispersion curves, Kreis 0Grad')
xlabel('Phase k')
ylabel('frequenzy f')
xlim([0 pi])
