...
%Point Loads
addPointLoad([node03, node08, node13],10,[1 -1 0]);
addPointLoad([node09, node19],20,[1 -1 0]);
addPointLoad([node11, node16, node21],10,[1 -1 0]);
addPointLoad([node23],15,[1 -1 0]);
...
eleIntf = [ele43, ele46, ele49, ele52]; 
[substructure01, substructure02] = modelNine.divide(eleIntf,idt);


%find new bar element with ids...
elements = substructure01.getAllElements;
eleIntf = [];
for ii = 1:length(elements)
    if elements(ii).getId == 10 || elements(ii).getId == 24 || elements(ii).getId == 37
