...
v=2;
hz=3;
nodematrixtest=substructureFETI_DP2.setupNodeMatrix(model,dim);
[K,bc,br,in,gbc,gbr,gin]=substructureFETI_DP2.substructureNodeMatrix(nodematrixtest,Ns,v,hz,dim);
%[DoubleNodes]=substructureFETI_DP.getDoubleNodes(gbr);
[sNodeIdArray]=substructureFETI_DP2.getSubstructureNodeIdArray(K,v,hz);
[sNodeArray] = substructureFETI_DP2.getSubstructureNodeArray(model,sNodeIdArray,v,hz);
[sElementArray,id,nodes] = substructureFETI_DP2.getSubstructureElementArray(model,sNodeIdArray,v,hz);
[sDofArray]= substructureFETI_DP2.getSubstrucureDofArray(sNodeArray,v,hz);
[gstiffnessMatrix, greducedStiffnessMatrix] = substructureFETI_DP2.assembleSubstructureStiffnessMatrix(model,sElementArray,sDofArray,gbr,v,hz);
[Ksort,Krr,Kcc,Krc,Kcr,suDofId,srDofId,suDofIdLoc,srDofIdLoc]=substructureFETI_DP2.splitMatrix(model,gstiffnessMatrix,sDofArray,v,hz,in,br,bc);
[sForceVector,ubcId]=substructureFETI_DP2.getSubstructureForceVector(model,assembling,suDofId,gbc,gbr,v,hz);
[gfr,gfbc]=substructureFETI_DP2.sortSubstructureForceVector(sForceVector,srDofId,v,hz);
[Bbr,ur,ur2,sinDofId,sbrDofId]=substructureFETI_DP2.getInterfaceBooleanMatrix(model,in,sDofArray,srDofId,v,hz);
[Bc,bcgl,bcdofId]=substructureFETI_DP2.getCornerBooleanMatrix(model,sDofArray,bc,gbc,hz,v);
[FIrr,FIrc,Kcc,Kccg,dr,fcg]=substructureFETI_DP2.assembleAllParameters(v,hz,Kcc,Krc,Krr,Bc,Bbr,gfr,gfbc);
[Kbrbr]=substructureFETI_DP2.getBoundryReminderMatrix(Krr,sinDofId,v,hz);
[lP]=substructureFETI_DP2.getLumpedPreconditioner(Bbr,Kbrbr,sinDofId,srDofId,ur2,v,hz);
[lPglobal]=substructureFETI_DP2.assembleLumpedPreconditioner(lP,v,hz);
[lmd]=FETI_DPSolver2.PCG(FIrr,FIrc,Kccg,dr,fcg,lPglobal);
[uc]=FETI_DPSolver2.solveCornerDofs(Kccg,fcg,FIrc,lmd);
[urem]=FETI_DPSolver2.solveReminderDofs(Krr,gfr,Krc,Bc,uc,Bbr,lmd,v,hz);
[ufinal,ufinalred]=FETI_DPSolver2.getResultVector(model,uc,urem,ur,ur2,bcdofId,v,hz);
