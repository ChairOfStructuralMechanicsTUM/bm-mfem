...
%VerschiebungDofs_dazel = model.getDofArray.getValue(step);
nodalForces_dazel = solver.getNodalForces(step);
  

%DAZELPLOT = figure('Name','DAZELPlot');
...
LAME_LAMBDA = (1+1i*p.ETA_S)*p.LAMBDA_S;
LAME_MUE = (1+1i*p.ETA_S)*p.MUE_S;

POISSON_RATIO = LAME_LAMBDA/2*(LAME_LAMBDA+LAME_MUE)
K = LAME_LAMBDA*(1+POISSON_RATIO)/(3*POISSON_RATIO)
...
DisplacementDifferenceNonPorous(:,1) = DisplacementAllardSolid(:,2) - DisplacementHomogenousSolid(:,2);
DisplacementDifferenceNonPorous(:,2) = DisplacementAtallaSolid(:,2) - DisplacementHomogenousSolid(:,2);
DisplacementDifferenceNonPorous(:,3) = DisplacementDazelSolid(:,2) - DisplacementHomogenousSolid(:,2);
