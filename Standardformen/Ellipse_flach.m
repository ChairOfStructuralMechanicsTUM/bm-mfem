%%% A = 20 cm^2; vertRadius=7.5cm ; horRadius=20/(7.5*pi)cm

clear

io=MdpaInput('Ellipse_flach.mdpa'); %specify input file
...
% % 
% % model.getElement(199).setPropertyValue('ELEMENTAL_STIFFNESS',7e8);
% % model.getElement(199).setPropertyValue('ELEMENTAL_DAMPING',0);
% % model.getElement(200).setPropertyValue('ELEMENTAL_STIFFNESS',7e8);
...
for j = 1:nob
    ...
    figure(2)
    plot(kx,f(j,:),'r')
    title('Dispersion curves - Ellipse, flach ')
    xlabel('Phase k')
    ylabel('frequenzy f')
    xlim([0 pi])
