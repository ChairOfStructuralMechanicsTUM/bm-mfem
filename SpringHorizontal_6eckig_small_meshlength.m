    %%%% meshlänge: 0.01
    %%%% ergebnis sogar noch um 5-10 % höher als SEHR GUTES Ergebnis
    
    
clear

io=MdpaInput('small_meshes.mdpa'); %specify input file
...
%%% 2 springs, 1 mass
model.addNewNode(320,0.1,0.075,0);
model.addNewElement('SpringDamperElement3d2n',277,[94 320]);
model.addNewElement('SpringDamperElement3d2n',278,[320 161]);
model.addNewElement('ConcentratedMassElement3d1n',279, 320);

model.getNode(94).addDof("DISPLACEMENT_Z");
model.getNode(161).addDof("DISPLACEMENT_Z");
model.getNode(320).addDof(["DISPLACEMENT_X","DISPLACEMENT_Y","DISPLACEMENT_Z"]);


model.getNode(94).fixDof('DISPLACEMENT_Z');
model.getNode(161).fixDof('DISPLACEMENT_Z');
model.getNode(320).fixDof('DISPLACEMENT_Y');
model.getNode(320).fixDof('DISPLACEMENT_Z');


model.getElement(277).setPropertyValue('ELEMENTAL_STIFFNESS',8.6359e+8);
model.getElement(277).setPropertyValue('ELEMENTAL_DAMPING',0);
model.getElement(278).setPropertyValue('ELEMENTAL_STIFFNESS',8.6359e+8);
model.getElement(278).setPropertyValue('ELEMENTAL_DAMPING',0);

model.getElement(279).setPropertyValue('ELEMENTAL_MASS',7e0);
model.getElement(279).setPropertyValue('VOLUME_ACCELERATION',10);
