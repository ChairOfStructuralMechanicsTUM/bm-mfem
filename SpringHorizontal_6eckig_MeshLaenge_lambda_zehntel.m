    %%%% mit unstructured, nicht wirklich lambda zehntel (siehe Mesh)
    %%%% gute Ergebnisse
    
    
clear

io=MdpaInput('SpringHorizontal_6eckig_MeshLaenge_lambda_zehntel.mdpa'); %specify input file
...
%%% 2 springs, 1 mass
model.addNewNode(285,0.1,0.075,0);
model.addNewElement('SpringDamperElement3d2n',254,[133 285]);
model.addNewElement('SpringDamperElement3d2n',255,[285 77]);
model.addNewElement('ConcentratedMassElement3d1n',256, 285);

model.getNode(133).addDof("DISPLACEMENT_Z");
model.getNode(77).addDof("DISPLACEMENT_Z");
model.getNode(285).addDof(["DISPLACEMENT_X","DISPLACEMENT_Y","DISPLACEMENT_Z"]);


model.getNode(133).fixDof('DISPLACEMENT_Z');
model.getNode(77).fixDof('DISPLACEMENT_Z');
model.getNode(285).fixDof('DISPLACEMENT_Y');
model.getNode(285).fixDof('DISPLACEMENT_Z');


model.getElement(254).setPropertyValue('ELEMENTAL_STIFFNESS',8.6359e+8);
model.getElement(254).setPropertyValue('ELEMENTAL_DAMPING',0);
model.getElement(255).setPropertyValue('ELEMENTAL_STIFFNESS',8.6359e+8);
model.getElement(255).setPropertyValue('ELEMENTAL_DAMPING',0);

model.getElement(256).setPropertyValue('ELEMENTAL_MASS',7e0);
model.getElement(256).setPropertyValue('VOLUME_ACCELERATION',10);
...
% m = model.getElement(155).getPropertyValue('ELEMENTAL_MASS');
% k1 = model.getElement(153).getPropertyValue('ELEMENTAL_STIFFNESS');
% k2 = model.getElement(154).getPropertyValue('ELEMENTAL_STIFFNESS');
% 
